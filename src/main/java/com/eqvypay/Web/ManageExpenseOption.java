...
public class ManageExpenseOption {
	...
	public boolean expenseOptions(User user,ExpenseRepository expenseRepository) throws Exception {
        ...
        while (true) {
            ...
            if(option == 1) {
                ...
                if(payOption == 1) {
                	...
                	List<User> friends = new ArrayList<>(); // friendRepository.findAllFriends(String userId)
                	int count = 1;
                	System.out.format("%s", "Your added friends list is below");
                	for(User friend:friends) {
                		System.out.format("%20s %20s %20s", count,friend.getName(),friend.getEmail());
                	}
