...
import com.eqvypay.Service.ProfileService;
...
public class CommandLineRunner {
	...
	public void commandLineRunner() throws Exception {
		...
		while (true) {
			...
			case 7:
				User user2 = new User();
				user2.setContact("7806404405");
				user2.setEmail("prit");
				user2.setName("prit");
				user2.setPassword("asd");
				ProfileService profileService = new ProfileService();
				profileService.getProfile(user2);
				
				System.out.println("\nWould you like to update your profile details? (Y/N)");
				String needUpdate = scanner.nextLine();
				
				if(needUpdate.equalsIgnoreCase("y")) {
					profileService.updateProfile(user2);
				}
				break;
			default:
				System.out.println("\nUnknown Input. Please try again!\n");
